...
async def audio_to_transactions(audio_data: dict, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        prompt = f"""You are a financial transaction parser. Hoy es {today}.
Convert the spoken input into structured transaction data.
...
- category_id: Map to the closest Category ID from the list. If unknown, use null.
- account_id: Map to the closest Account ID from the list (payment method mentioned). If unknown, use null.
- description: Concise in Spanish.
- date: YYYY-MM-DD. Use {today} if not specified.
- IGNORE PII (names, emails, etc).
