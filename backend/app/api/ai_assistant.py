...
"""
        ...
        if request.document_base64:
            ...
            response = await with_gemini_retry_async(lambda: chat.send_message([doc_part, request.message]))
        else:
            response = await with_gemini_retry_async(lambda: chat.send_message(request.message))
        ...
        while response.function_calls and turn < max_turns:
            ...
            response = await with_gemini_retry_async(lambda: chat.send_message(cast(Any, tool_responses)))
            turn += 1
